...
export async function runFaxWorkerOnce(): Promise<number> {
  ...
  const { data: logRow } = await supabaseAdmin
    .from("workers_log")
    .insert({ worker_name: "fax-worker", job_type: "referral_outreach", status: "running" })
    .select("id")
    .maybeSingle();
  ...
  try {
    ...
    if (remaining <= 0) {
      await finishLog(logId, startedAt, 0, "daily limit reached");
    ...
    await finishLog(logId, startedAt, sent);
    console.log(`[fax-worker] sent ${sent} fax(es) of ${remaining} allowed today.`);
  } catch (err) {
    await failLog(logId, startedAt, err);
    console.error("[fax-worker] run failed:", err);
  }
...
async function failLog(logId: string | null, startedAt: number, err: unknown): Promise<void> {
  if (!logId) return;
  await supabaseAdmin
    .from("workers_log")
    .update({
