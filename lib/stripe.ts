...
/**
 * Ensure a patient has a Stripe customer.
 * The patients table has no stripe_customer_id column, so the customer is
 * looked up by email on Stripe (falling back to creating a new one).
 */
export async function ensureCustomer(patient: ChargePatient): Promise<string> {
  ...
  if (patient.email) {
    const existing = await getClient().customers.list({
      email: patient.email,
      limit: 1,
    });
